...
# MAGIC %md
# MAGIC Let's look at a subset of our data. We'll use the [sample()](https://spark.apache.org/docs/latest/api/python/pyspark.sql.html#pyspark.sql.DataFrame.sample) method to sample 10% of the DataFrame without replacement, and call `display()` on the resulting DataFrame.
...
# MAGIC - Use [VectorAssembler](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.VectorAssembler), and set `inputCols` to `featureCols`, and the `outputCols` as `rawFeatures`. This concatenates all feature columns into a single feature vector into the new column "rawFeatures".
# MAGIC - Use [VectorIndexer](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.feature.VectorIndexer) to identify categorical features in our `rawFeatures` and index them. If any column has `maxCategories` or fewer distinct values, then it is treated as a categorical variable.
...
# MAGIC %md
# MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Decision Tree Regressor
# MAGIC Second, we define the model training stage of the Pipeline. [Decision Tree Regressor](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.regression.DecisionTreeRegressor) takes feature vectors and labels as input and learns to predict labels of new examples.
# MAGIC 
...
# MAGIC %md
# MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Train
# MAGIC 
...
# MAGIC %md
# MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Predictions
# MAGIC 
...
# ANSWER
predictionsDF = savedModel.transform(testDF)
...
# MAGIC 
# MAGIC Next, we'll use [RegressionEvaluator](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.evaluation.RegressionEvaluator) to assess the results. The default regression metric is RMSE.
