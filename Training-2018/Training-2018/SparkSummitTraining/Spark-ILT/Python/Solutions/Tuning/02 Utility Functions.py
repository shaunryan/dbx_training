...
# MAGIC %md
# MAGIC This is actually shorthand for the following piece of code:

# COMMAND ----------

path = 'dbfs:/mnt/training'
...
print('Collection Type: {}'.format(type(files)))

first = files[0]
print('Contained Type:  {}'.format(type(first)))
...
# MAGIC %md
# MAGIC As a side note, there are actually a number of different commands for working with the file system.
# MAGIC 
# MAGIC See **dbutils.fs.help** as show below:  
...
# MAGIC %md
# MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) The Challenge
# MAGIC 
# MAGIC **You have 10 minutes to determine how many individual files make up our parquet file and its total size:**
# MAGIC * `dbfs:/mnt/training/asa/flights/1990-1999-by-year.parquet`
# MAGIC 
# MAGIC ** Requirements:**
...
# MAGIC 
# MAGIC ###Strategies:
# MAGIC * **Option #1**: Use **%fs ls dbfs:/mnt/training/asa/flights/1990-1999-by-year.parquet**, manually count every file, repeat for every subdirectory, and then add them all up.
# MAGIC * **Option #2**: Use **dbutls.fs.ls(..)** to programmatically sum and/or count the number of files in each directory.
# MAGIC   * **2.A:** Use Scala's native functions (or Pythons) to sum the count of all files.
...
# ANSWER
path = 'dbfs:/mnt/training/asa/flights/1990-1999-by-year.parquet'

(totalCount, totalBytes) = computeFileStats(path)

print("{:,} files".format(totalCount))
print("{:,} bytes".format(totalBytes))
...
validateYourAnswer("01) Total Count", 1680788708, totalCount)
validateYourAnswer("02) Total Bytes", 1928383137, totalBytes)
...
(count, bytes) = computeFileStats(path)
...
path = "dbfs:/mnt/training/asa/flights/1990-1999-by-year.parquet"
dfA = spark.read.parquet(path)

(dfB, total, duration) = benchmarkCount(lambda: df)

print("Query Duration: {:,} ms".format(int(duration)))
print("-"*80)
