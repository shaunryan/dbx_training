...
# MAGIC 
# MAGIC I need to tell you something: We are going to induce a bug in Databricks.
# MAGIC 
...
# MAGIC %md
# MAGIC If the assertion above fails:
# MAGIC * Edit your cluster accordingly
# MAGIC * Restart the cluster
# MAGIC * Rerun the previous cell to test it
...
# MAGIC 
# MAGIC Embeded in the description of every transaction is a wealth of information.
# MAGIC 
...
postedOnPattern = "(.*\S)(\s*)(\d\d-\d\d)"

postedOnTempDF = fast2017DF.withColumn("postedOn", when(regexp_extract(col("description"), postedOnPattern, 3) != "", regexp_extract(col("description"), postedOnPattern, 3)).otherwise(lit(None)))
...
condition = when(col("postedOn").isNotNull(), regexp_extract(col("description"), postedOnPattern, 1)). \
            otherwise(col("description"))

postedOnDF = (postedOnTempDF.withColumn("description", condition)
)
...
# MAGIC 
# MAGIC Normally we would do only after the pattern emerges.
# MAGIC 
...
def extractValue(df, columnName, pattern, group):
  return df.withColumn(columnName, when(regexp_extract(col("description"), pattern, group) != "", regexp_extract(col("description"), pattern, group)).otherwise(lit(None)))
...
postedOnTempDF = extractValue(df=fast2017DF, columnName="posted_on", pattern=postedOnPattern, group=3)
...
count = postedOnTempDF.select("posted_on").distinct().count()
...
def updateDescription(df, columnName, pattern):
  return df.withColumn("description", when(col(columnName).isNotNull(), regexp_extract(col("description"), pattern, 1)).otherwise(col("description")))
...
postedOnDF = updateDescription(df=postedOnTempDF, columnName="posted_on", pattern=postedOnPattern)
...
count = postedOnDF.filter(col("description").endswith("11-25")).count()
...
locationTempDF = extractValue(postedOnDF, "location", locationPattern, 2)
locationDF = updateDescription(locationTempDF, "location", locationPattern)
...
trxPattern = "(.*\S)(.*\W\W)([a-z][a-z][a-z])\sid:(\W[0-9]+)"

trxIdTempADF = extractValue(locationDF, "transactionId", trxPattern, 4)
trxIdTempBDF = extractValue(trxIdTempADF, "transactionType", trxPattern, 3)
trxIdDF = updateDescription(trxIdTempBDF, "transactionId", trxPattern)
...
cleanDF = (trxIdDF
  .withColumn("retailer", when(trim(col("description")) == "unkn", lit(None)).otherwise(trim(col("description"))))
  .drop("description")
)
...
# MAGIC * The method should return the new **DatFrame**
...
def categorize(df, category, retailers): 
  return df.withColumn("category", when(col("retailer").isin(retailers), lit(category)).otherwise(col("category")))

finalDF = cleanDF.withColumn("category", when(col("retailer") == "null", lit("Miscellaneous")).otherwise(lit("null")))
finalDF = categorize(finalDF, "Miscellaneous",     ["Amazon.com", "Signet Jewelers"])

finalDF = categorize(finalDF, "Drug Store",        ["CVS", "Walgreen","Rite Aid", "Alimentation Couche-Tard"])
finalDF = categorize(finalDF, "Convenience Store", ["7-Eleven"])
finalDF = categorize(finalDF, "Dining Out",        ["YUM! Brands", "McDonald's", "Wendy's", "Subway", "BJ's", "DineEquity","Starbucks"])
finalDF = categorize(finalDF, "Entertainment",     ["iTunes", "GameStop", "Dick's Sporting Goods","Michael's"])
finalDF = categorize(finalDF, "Grocery",           ["Safeway", "Albertsons", "Trader Joe's", "Costco", "Meijer", "Kroger", "Ahold","Publix Super Markets"])
finalDF = categorize(finalDF, "House Hold",        ["Wal-Mart", "Sears", "Target", "Ikea","AVB Brandsource"])
finalDF = categorize(finalDF, "Home Improvement",  ["Home Depot", "Ace Hardware", "Menard"])
finalDF = categorize(finalDF, "Clothing",          ["Macy's", "TJ Max", "Kohl's", "J.C. Penney"])
...
display(
  finalDF.filter(col("category") != "null")
)
...
# MAGIC * ~40 seconds
# MAGIC * ~1.25 minutes
# MAGIC * It can very rappidly get up to 10 minutes.
# MAGIC 
...
# MAGIC         <li>Note #2: Adding more wide transformations will also increase execution time however narrow transformations generally should have little-to-no affect on execution time.</li>
# MAGIC 
# MAGIC         <p>
# MAGIC           <button style="width:3em" onclick="inline('push-it', 'not-prog')">Yes</button>
# MAGIC           &nbsp;
# MAGIC           <span id="push-it" style="display:none">If the degradation is progressive, you should be able to push it to fail - this can help to further diagnoise the problem.</span>
# MAGIC         </p>
# MAGIC 
...
# MAGIC       <div style="margin-top:1em"><button style="width:15em" onclick="block('wa', 'ca')">In an Executors</button></div>
# MAGIC 
...
# MAGIC         <div style="margin-top:1em">What this means is that the driver has used all of it's heap space.</div>
# MAGIC         <div style="margin-top:1em; font-weight:bold; color:green">Go to the next step</div>
# MAGIC       </div>
# MAGIC 
...
# MAGIC 
# MAGIC       <div style="margin-top:1em">In all honesty, this is where tradition trouble shooting starts to fail us.</div>
# MAGIC 
# MAGIC       <div style="margin-top:1em">
...
# MAGIC         <li>The transformations we are executing are faily strait forward</li>
...
# MAGIC 
# MAGIC       <div style="margin-top:1em">If you look at the Logical Plan, you can see that it is really strait forward.</div>
...
# MAGIC         <div style="margin-top:1em">Especially when we are seeing a trippling of execution time with every new transformation?</div>
# MAGIC       </div>
# MAGIC       
...
# MAGIC 
# MAGIC       <div style="margin-top:1em">At this stage it's a fair conclustion.</div>
# MAGIC 
...
whenCol = ( when(col("retailer") == "null", lit("Miscellaneous"))
             .when(col("retailer").isin(["Amazon.com", "Signet Jewelers"]), lit("Miscellaneous"))
             .when(col("retailer").isin(["CVS", "Walgreen","Rite Aid", "Alimentation Couche-Tard"]), lit("Drug Store"))
             .when(col("retailer").isin(["7-Eleven"]), lit("Convenience Store"))
             .when(col("retailer").isin(["YUM! Brands", "McDonald's", "Wendy's", "Subway", "BJ's", "DineEquity","Starbucks"]), lit("Dining Out"))
             .when(col("retailer").isin(["iTunes", "GameStop", "Dick's Sporting Goods","Michael's"]), lit("Entertainment"))
             .when(col("retailer").isin(["Safeway", "Albertsons", "Trader Joe's", "Costco", "Meijer", "Kroger", "Ahold","Publix Super Markets"]), lit("Grocery"))
             .when(col("retailer").isin(["Wal-Mart", "Sears", "Target", "Ikea","AVB Brandsource"]), lit("House Hold"))
             .when(col("retailer").isin(["Home Depot", "Ace Hardware", "Menard"]), lit("Home Improvement"))
             .when(col("retailer").isin(["Macy's", "TJ Max", "Kohl's", "J.C. Penney"]), lit("Clothing"))
             .otherwise(lit("null"))
)

finalDF = cleanDF.withColumn("category", whenCol)
