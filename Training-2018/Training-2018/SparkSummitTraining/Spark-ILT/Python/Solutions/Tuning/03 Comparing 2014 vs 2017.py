...
trx2014DF.sample(False, 0.10, 42) \
         .write.parquet(fastPath14)
  
trx2017DF.sample(False, 0.10, 42) \
         .write.parquet(fastPath17)
...
byMonth2017DF = (fast2017DF
  ...
  .withColumn("month", month(col("transacted_at")))
  .groupBy("year", "month")
  .sum("amount")
  .withColumnRenamed("sum(amount)", "amount")
  .orderBy("year", "month")
...
holiday14DF = (trx2014DF # fast2014DF or trx2014DF
  # no need to extract the month from transacted_at
  # no need to extract the year from transacted_at
  .filter((col("year") == 2014) & (col("month") >= 11))
...
holiday17DF = (trx2017DF # fast2017DF or trx2017DF
  .withColumn("month", month("transacted_at"))
  .withColumn("year", year("transacted_at"))
  .filter((col("year") == 2017) & (col("month") >= 11))
  .withColumn("day", dayofyear("transacted_at"))
  .groupBy("year", "day").sum("amount")
  .withColumnRenamed("sum(amount)", "amount")
  .orderBy("day")
