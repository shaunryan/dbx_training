...
sock.connect(("server1.databricks.training", 9002))
buf = []
try:
  eof = False
  while not eof:
...
jsonString = ''.join(buf)
print(json.dumps(json.loads(jsonString), sort_keys=True, indent=2))      
...
def prettySchema(schema, indentation=2):
...
inferredSchema = spark.read.json(sc.parallelize([jsonString])).schema
print(prettySchema(inferredSchema))
...
jsonEdits = editsDF.select(col("timestamp"), from_json(col("value"), schema).alias("json"))
...
editsFinalDF = (jsonEdits
    .select(col("json.wikipedia").alias("wikipedia"),
            ...
            unix_timestamp(col("json.timestamp"), "yyyy-MM-dd'T'HH:mm:ss.SSSX").cast("timestamp").alias("timestamp"),
...
anonDF = (editsFinalDF
  ...
  .filter(~ isnull(col("geocoding.countryCode3")))
)
...
# MAGIC 
# MAGIC <img src="https://files.training.databricks.com/images/structured-streaming-world-graph-plot-options-1.png" style="float: right; height: 400px"/>
# MAGIC 
...
display(
  anonDF
    .groupBy( window( col("timestamp"), "1 second", "500 milliseconds"), 
              col("geocoding.countryCode3"))
    .count()
    .select(col("countryCode3"), col("count"))
)

# COMMAND ----------

# MAGIC %md
# MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/labs.png) Structured Streaming Lab
