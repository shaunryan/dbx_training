...
path2018 = FILL_IN   # specify the path to the 2018 parquet "file"
trx2018DF = FILL_IN  # read in the 2018 parquet file

fast18Path = "{}/2018-fast.parquet".format(userhome)

(trx2018DF.FILL_IN   # Sample the data: 10% without replacement
          .FILL_IN   # create the DataFrameWriter
          .FILL_IN)  # write out the parquet file

fast2018DF = FILL_IN # Read in the "fast" parquet file
...
expectedFast18 = "447558919.47"

holidayFast18DF = sumByDayOfYear(fast2018DF)
...
expectedSlow18 = "4471732275.67"

holidaySlow18DF = sumByDayOfYear(trx2018DF)
...
fast2017DF = spark.read.parquet("{}/2017-fast.parquet".format(userhome))
...
allFastDF = holidayFast18DF.unionByName(holidayFast17DF).unionByName(holidayFast14DF)
...
trx2017DF = spark.read.parquet(path2014)
...
allSlowDF = holidaySlow18DF.unionByName(holidaySlow17DF).unionByName(holidaySlow14DF)
display(allFastDF.select("day", "amount", "year"))
...
print("2014 Count: {:,} records\n".format(trx2014DF.count()))

# COMMAND ----------

print("2018 Count: {:,} records\n".format(trx2018DF.count()))
...
# MAGIC         This often occurs as we add more data - the amount of data between 2018 and 2014 is not enough to explain a jumpt from ~1 minute to 10+ minutes.
# MAGIC       </p>
# MAGIC       
# MAGIC     </div>
# MAGIC   </body>
...
# MAGIC     <li style="color:green; font-weight:bold">10s of GB per partition (~10-50GB)</li>
...
# MAGIC     <li style="color:green; font-weight:bold">100s of MBs - 1GB ideal (usually 1GB)</li>
# MAGIC   </ul>
# MAGIC 
# MAGIC </div>
...
# MAGIC <style>td {padding:0} table { border-spacing: 0; border-collapse: collapse; }</style>
# MAGIC <div style="display:table; height:600px; width:100%">
# MAGIC   <h2><img src="https://files.training.databricks.com/images/105/logo_spark_tiny.png"/> Review Challenge #2B, The 4Ses</h2>
