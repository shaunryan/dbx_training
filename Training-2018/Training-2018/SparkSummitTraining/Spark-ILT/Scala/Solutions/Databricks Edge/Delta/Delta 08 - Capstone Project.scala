...
val outputPathBronze = userhome + "/gaming/bronze"
val outputPathSilver = userhome + "/gaming/silver"
val outputPathGold = userhome + "/gaming/gold"
...
// MAGIC %md
// MAGIC Wait until the stream is initialized, then create table `mobile_events_delta_raw`.
...
lazy val actualSchema = spark.table("mobile_events_delta_query").schema
...
dbTest("Delta-08-querySchema", Set(expectedSchema), Set(actualSchema), "Schemas do not match")
...
lazy val WAUactualSchema = spark.table("mobile_events_delta_summary").schema

lazy val WAUexpectedSchema = StructType(
  List(
...
// ANSWER  
val deviceId = spark.sql("SELECT deviceId FROM mobile_events_delta_query limit 1").collect()(0)(0).toString
...
// ANSWER
spark.sql("DELETE FROM mobile_events_delta_query WHERE deviceId='$deviceId' ")
val noDeviceId = spark.sql("SELECT * FROM mobile_events_delta_query WHERE deviceId='$deviceId' ").collect()
