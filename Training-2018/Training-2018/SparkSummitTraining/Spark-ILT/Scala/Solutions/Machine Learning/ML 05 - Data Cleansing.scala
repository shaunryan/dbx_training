...
val filePath = "/mnt/training/data-cleansing/bookreviews-5.csv"
...
val priceDF = rawDF.withColumn("price", regexp_replace($"price", "[\\$,]", "").cast("Double"))
display(priceDF)
...
val noCorruptRowsDF = priceDF.na.drop(Array("id"))
...
val imputeCols = Array("reviewsAvg")
imputer.setInputCols(imputeCols)
imputer.setOutputCols(imputeCols)

val imputedDF = imputer.fit(noCorruptRowsDF).transform(noCorruptRowsDF)
...
val cleanDF = imputedDF.na.fill("<<MISSING REVIEWS>>",Array("topReview"))
