...
val Array(trainingDF, testDF) = ratingsDF.randomSplit(Array(0.8, 0.2), seed=seed)
...
print("Baseline RMSE: {0:.3}".format(regEval.evaluate(benchmarkDF)))
...
val predictedTestDF = myModel.transform(testDF)

// Run the previously created RMSE evaluator, regEval, on the predictedTestDF DataFrame
val testRmse = regEval.evaluate(predictedTestDF)

println(s"The model had a RMSE on the test set of $testRmse")
...
// ANSWER
val trainingWithMyRatingsDF = trainingDF.union(myRatingsDF)
...
val myRatingsModel = als.fit(trainingWithMyRatingsDF)
...
// MAGIC %sql
// MAGIC SELECT p.title, p.prediction AS your_pedicted_rating
