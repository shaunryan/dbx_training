...
// MAGIC %md
// MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Random Forests in Action
...
val predictedDF = rfModel.transform(testDF)
...
println("Number of trees trained: %d\nFeature importances in individual trees:\n".format(rfModel.trees.length))
rfModel.trees.zipWithIndex.map( t => {
  val ind = t._2
  val dtModel = t._1
  println("Decision Tree #%s:".format(ind + 1))
  val imp = dtModel.featureImportances.toArray
  featureCols
    .zip(imp.zipWithIndex)
    .filter(_._2._1 > 0)
    .sortBy(-_._2._1)
    .foreach(f => println((("  " + f._1 + " (id=" + f._2._2 + "): ").padTo(30," ").mkString + f._2._1)))
  println()
})
println("-"*80)
