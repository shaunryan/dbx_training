...
// MAGIC %md
// MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) K-Means in Action
...
val kmeansModel = kmeans.fit(vecDF)
...
val clusteredDF = kmeansModel.transform(vecDF)
display(clusteredDF)
...
val silhouette = evaluator.setPredictionCol("clusterid").evaluate(clusteredDF)
