...
// MAGIC %md-sandbox
// MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Import the GraphFrames library
// MAGIC 
...
// MAGIC    <img src="https://files.training.databricks.com/images/setup_notebook/Click-Workspace-Button.png"></img>
// MAGIC 
// MAGIC 2. At the top of the file browser, you'll see "Workspace" with a downward arrow next to it.  Click on the arrow.  
// MAGIC    <img src="https://files.training.databricks.com/images/setup_notebook/Workspace-Down-arrow.png" alt="Screenshot placeholder"></img>
// MAGIC 
// MAGIC 3. In the popup menu, select Create -> Library.  You'll then see the "New Library" screen.  
// MAGIC    <img src="https://files.training.databricks.com/images/setup_notebook/Create-Library.png" alt="Screenshot placeholder" style="border:1px solid black"></img>
// MAGIC 
...
// MAGIC 
// MAGIC <img src="https://files.training.databricks.com/images/301/GraphFramesLibrary.png" style="height: 400px; margin: 10px; border: 1px solid #ddd; border-radius: 15px 15px 15px 15px; padding: 10px"/>

// COMMAND ----------

// MAGIC %md
// MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Loading the GraphFrames library
// MAGIC 
...
// ANSWER
display(df.filter($"Wk" === 1).filter($"Rank_W".isNotNull).select($"Winner", $"Rank_W"))
...
// ANSWER
val dfWin = df.select($"Winner")
val dfLose = df.select($"Loser")
val vertices = dfWin.union(dfLose).withColumnRenamed("Winner", "id").distinct
...
val edges = df.select(col("Loser").alias("src"), col("Winner").alias("dst"))
...
// ANSWER
val winLossDF = (g.outDegrees).join(g.inDegrees, "id")
val winPercentageDF = winLossDF.selectExpr("id", "inDegree/(inDegree + outDegree) as winPercentage")
...
//ANSWER
val shortest_path = g.shortestPaths.landmarks(Seq("UCLA")).run()
...
// ANSWER
val results = g.pageRank.resetProbability(0.10).maxIter(8).run()
