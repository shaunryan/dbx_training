...
// MAGIC %md
// MAGIC COB should be disabled by default.
// MAGIC 
// MAGIC Just in case it was enabled, let's shut it off real quick:
...
// MAGIC 
// MAGIC // Make sure the optimizer is turned off (the default)
...
// MAGIC %md
// MAGIC Drop the tables just in case they already exists...
...
// MAGIC %md
// MAGIC 
// MAGIC ### According to the Spark UI, how far off was Spark on it's estimation of the final *SortMergeJoin*?
// MAGIC 
// MAGIC ### How much data from the *stores* table was broadcasted?
// MAGIC 
// MAGIC ### Why was the **cities** table not broadcasted? How large was it?
...
// MAGIC %md
// MAGIC ### What is the default threashold for broadcasting?
...
// MAGIC 
// MAGIC ### What was the size of the data that we expected to be broadcasted?
// MAGIC 
...
// MAGIC 
// MAGIC We only analyized the table, not specific columns.
// MAGIC 
// MAGIC If we give the CBO just a little more information it can do a better job.
// MAGIC 
