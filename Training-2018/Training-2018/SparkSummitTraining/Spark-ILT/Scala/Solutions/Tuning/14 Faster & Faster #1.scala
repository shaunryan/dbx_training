...
val tempCityPath = "%s/cityies-temp.parquet".format(userhome)
val tempTrxPath = "%s/trx-temp.parquet".format(userhome)

spark.read
     .parquet(citiesPath)
     .filter($"state".isNotNull || $"city" === "Vaiaku village, Funafuti province")
...
val finalDF = spark.read.parquet(tempTrxPath).join(spark.read.parquet(tempCityPath), "city_id")
  .groupBy("cityState", "country").sum("amount")
