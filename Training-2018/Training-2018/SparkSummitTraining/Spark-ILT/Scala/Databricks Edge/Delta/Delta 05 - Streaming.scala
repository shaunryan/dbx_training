...
val outputPath = userhome + "/gaming"
val basePath = userhome + "/advanced-streaming"
val checkpointPath = basePath + "/checkpoints"
...
display(countsDF.withColumn("hour",hour(col("window.start"))), "countsDisplay")

// COMMAND ----------

// TEST - Run this cell to test your solution.
import org.apache.spark.sql.types.{StructType, StructField, StringType, LongType, TimestampType, IntegerType}

lazy val countsExpectedSchema = StructType(List(
   StructField("gt", StringType, true),
   StructField("count", LongType, false),
   StructField("hour", IntegerType, true),
   StructField("window", StructType(List(
       StructField("start", TimestampType, true),
       StructField("end", TimestampType, true)
   )), false)
))

dbTest("Delta-05-schemas-gt", countsExpectedSchema("gt"), countsDF.schema("gt"))
dbTest("Delta-05-schemas-count", countsExpectedSchema("count"), countsDF.schema("count"))
dbTest("Delta-05-schemas-hour", countsExpectedSchema("hour"), countsDF.schema("hour"))
...
spark.streams.active.foreach(FILL_IN)
