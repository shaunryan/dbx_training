...
val topTenDF = spark                             // Our SparkSession & Entry Point
  ...
  .filter($"article" =!= "Main_Page")            // Transformation
  .filter($"article" =!= "-")                    // Transformation
  .filter( ! $"article".startsWith("Special:"))  // Transformation
...
// MAGIC * <a href="https://spark-summit.org/2016/events/deep-dive-into-catalyst-apache-spark-20s-optimizer" target="_blank">Deep Dive into Catalyst: Apache Spark 2.0's Optimizer</a>, Yin Huai's Spark Summit 2016 presentation.
