...
val (df, total2014Slow, duration14Slow) = benchmarkCount(() => trx2014DF)
...
val (df, total2017Slow, duration17Slow) = benchmarkCount(() => trx2017DF)
...
val fastPath14 = s"$userhome/2014-fast.parquet"
val fastPath17 = s"$userhome/2017-fast.parquet"

trx2014DF.FILL_IN        // write out a sample of the data to disk
trx2017DF.FILL_IN        // write out a sample of the data to disk

val fast2014DF = FILL_IN // read in the faster parquet file
val fast2017DF = FILL_IN // read in the faster parquet file
...
// val results = loadYourTestMap("dbfs:/mnt/training-rw/global-sales/solutions/sample-results")
...
val holiday14DF = fast2014DF // trx2014DF
  // no need to extract the month from transacted_at
  // no need to extract the year from transacted_at
...
val holiday17DF = fast2017DF // trx2017DF
  .FILL_IN  // extract the month from transacted_at
  .FILL_IN  // extract the year from transacted_at
  .FILL_IN  // limit the data to Nov & Dec
  .FILL_IN  // extract the day from transacted_at
