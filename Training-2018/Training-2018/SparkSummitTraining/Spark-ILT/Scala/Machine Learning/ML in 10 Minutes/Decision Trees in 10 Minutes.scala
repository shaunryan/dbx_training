...
// MAGIC 
// MAGIC The `spark.ml` implementation supports decision trees for binary and multiclass **classification and** for **regression**, using both continuous and categorical features. The implementation partitions data by rows, allowing distributed training with millions or even billions of instances.
// MAGIC 
// MAGIC Take a look at this <a href="http://www.r2d3.us/visual-intro-to-machine-learning-part-1/" target="_blank">amazing website</a> if you'd like to see a visual introduction of Decision Trees.
// MAGIC 
// MAGIC <small><i>Source: https://spark.apache.org/docs/latest/ml-classification-regression.html#decision-trees</i></small>
...
// MAGIC %md
// MAGIC * <a href='https://spark.apache.org/docs/latest/ml-classification-regression.html#decision-tree-classifier' target="_blank">Decision Tree Classifier</a> in the Spark ML programming guide
// MAGIC * <a href='https://spark.apache.org/docs/latest/ml-classification-regression.html#decision-tree-regression' target="_blank">Decision Tree Regressor</a> in the Spark ML programming guide
...
// MAGIC %md
// MAGIC ##![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Decision trees in Action
...
// MAGIC ### Our dataset
// MAGIC 
// MAGIC We are using the <a href="https://archive.ics.uci.edu/ml/datasets/wine">Wine Data Set</a> to train a model, which will predict wine quality by analysing the chemical attributes of different wine samples.
// MAGIC 
...
val df = spark.read.parquet("/mnt/training/wine.parquet")
display(df)

// COMMAND ----------

// MAGIC %md
// MAGIC Now, create a barchart that displays the distribution of the `quality` column:
...
// MAGIC %md
// MAGIC ### Training a Decision Tree Regressor

// COMMAND ----------

// MAGIC %md
// MAGIC Let's take a look at the expert parameters of Decision Trees
...
// MAGIC %md
// MAGIC Now, train a Decision Tree Regressor model:

// COMMAND ----------

import org.apache.spark.ml.feature.VectorAssembler

val featureCols = df.columns.filter(_ != "quality")
val assembler = new VectorAssembler()
                    .setInputCols(featureCols)
                    .setOutputCol("features")
val vecDF = assembler.transform(df)

val Array(trainingDF, testDF) = vecDF.randomSplit(Array(0.8, 0.2), seed=273)
(dt.setLabelCol("quality")
   .setMaxDepth(3)
)
val dtModel = dt.fit(vecDF)

// COMMAND ----------

// MAGIC %md
// MAGIC Let's evaluate our Decision Tree Regressor Model:

// COMMAND ----------

import org.apache.spark.ml.evaluation.RegressionEvaluator

val predictedDF = dtModel.transform(testDF)

val eval = new RegressionEvaluator()
           .setLabelCol("quality")
           .setMetricName("mae")

println("Mean Absolute Error: %1.2f".format(eval.evaluate(predictedDF)))
println("-"*80)

// COMMAND ----------

// MAGIC %md
// MAGIC What are the features with the most predictive power in this dataset?

// COMMAND ----------

val imp = dtModel.featureImportances.toArray
featureCols
  .zip(imp.zipWithIndex)
  .filter(_._2._1 > 0)
  .sortBy(-_._2._1)
  .foreach(f => println(((f._1 + " (id=" + f._2._2 + "): ").padTo(28," ").mkString + f._2._1)))
println("-"*80)

// COMMAND ----------

// MAGIC %md
// MAGIC ### Visualizing the Decision Tree
// MAGIC 
// MAGIC A graphical representation of the tree is only available in Scala at the moment. In Python you can use the `df.toDebugString()` function to see the internal structure of the tree. 

// COMMAND ----------

dtModel.write.overwrite.save(userhome + "/decisiontree-winequality.sparkmodel")

// COMMAND ----------

// MAGIC %scala
// MAGIC import org.apache.spark.ml.regression.DecisionTreeRegressionModel
// MAGIC val savedModel = DecisionTreeRegressionModel.load(userhome + "/decisiontree-winequality.sparkmodel")
// MAGIC display(savedModel)
