...
val cnt = rawDF.count()
assert (cnt == 4804, "Number of records, expected " + 4804 + " found " + cnt)
assert (rawDF.schema(0).dataType == IntegerType, "Column `id` excepted to be IntegerType, got " + 
        rawDF.schema(0).dataType)
...
// TODO: Replace <FILL_IN> with appropriate code
val noNullsDF = fixedPriceDF.na.<FILL_IN>
...
val cnt = noNullsDF.count()
assert (cnt == 4746, "Number of records, expected " + 4746 + " found " + cnt)
println("Tests passed.")
println("-"*80)
...
val imputeCols = Array(<FILL_IN>)

val imputer = <FILL_IN>
imputer.<FILL_IN> // Set strategy, inputCols, outputCols

imputedDF = imputer.<FILL_IN> // Fit and transform on doublesDF
...
val impCnt = imputedDF.count()
val noNullsCnt = imputedDF.na.drop().count()
...
// TODO: Replace <FILL_IN> with appropriate code
val cleanDF = posPricesDF.<FILL_IN>
