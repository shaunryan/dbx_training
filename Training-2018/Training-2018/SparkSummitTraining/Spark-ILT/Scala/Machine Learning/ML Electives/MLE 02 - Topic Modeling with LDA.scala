...
val corpusRawDF = spark.read.parquet("/databricks-datasets/news20.binary/data-001/training")
...
val messageCnt = corpusRawDF.count()
val topicCount = corpusRawDF.select($"topic").distinct().count()
printf("The DataFrame contains %d emails accross %d topics.%n%n", messageCnt, topicCount )
...
val topicsDF = corpusRawDF.select($"topic").distinct()
display(topicsDF)
...
val tokenizedDF = tokenizer.transform(corpusRawDF)
...
val nntp_stopwords = Array("writes", "subject", "from", "organization", "line", "lines", "entry", "date", "nntp", "host", "reply", "posting", "newsgroup")

// Additional common words in this corpus
val additional_stopwords = Array("article", "entry", "udel", "said", "tell", "think", "know", "just", "like", "does", "going", "make", "thanks", "people")

// Update the stopwords dictionary and apply the StopWordsRemover
val new_stopwords = stopwords ++ nntp_stopwords ++ additional_stopwords
remover.setStopWords(new_stopwords)

val noStopWordsDF = remover.transform(tokenizedDF)

// Show a sample of short messages on the screen and see which words got removed.
val shortMessagesDF = noStopWordsDF.filter(length($"text") < 200)
...
val vectorizer = new CountVectorizer()
                     .<FILL_IN>  // set input column 
                     .<FILL_IN>  // set output column
                     .<FILL_IN>  // set vocabulary size
                     .<FILL_IN>  // set the minimum DF to 5 
...
pipeline=<FILL_IN> // Create a Pipeline object
pipeline.<FILL_IN> // Set stages

val pipelineModel = pipeline.<FILL_IN> // Fit

val ldaModel = pipelineModel.stages.last.asInstanceOf[LDAModel]
...
val topicsAndTermsDF = ldaModel.describeTopics(maxTermsPerTopic=3)
...
val topicsLocal = topicsAndTermsDF.collect()

println(topicsLocal(0))
println("-"*80)

// COMMAND ----------

import org.apache.spark.ml.feature.CountVectorizerModel
val vectorizerModel = pipelineModel.stages(2).asInstanceOf[CountVectorizerModel] // The model of the vectorizer, which contains the termIndex-term vocabulary
val voc = vectorizerModel.vocabulary

val raw_json = topicsLocal.flatMap { row => 
  val topicId = row.getInt(0)
  val termIndices = row.getSeq(1).asInstanceOf[Seq[Int]]
  val termWeights = row.getSeq(2).asInstanceOf[Seq[Double]]
  termIndices zip termWeights map { t => 
    s"""{"topicId": "$topicId", "term": "${voc(t._1)}", "probability": "${t._2}"}""" 
  }}.mkString(",\n")
