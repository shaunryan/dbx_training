...
val peopleDFLastName = peopleDF.orderBy("lastName").select("lastName")

peopleDFLastName.count
...
peopleDFLastName.limit(10000).count
...
peopleDFLastName.explain()

// COMMAND ----------

peopleDFLastName.limit(10000).explain()
...
def checkPartitions(df: DataFrame): Unit = {
  println(s"Number of partitions = ${df.rdd.getNumPartitions}\n")

  df
    .rdd
    .mapPartitionsWithIndex( {(index, iterator) => Array((index, iterator.size)).iterator}, true )
    .collect
    .foreach { case (index, size) =>
      println(f"Partition $index%2d: $size bytes")
    }
}

checkPartitions(peopleDFLastName)
...
checkPartitions(peopleDF.orderBy("lastName").select("lastName"))
...
val peopleDF = spark.read.parquet("dbfs:/mnt/training/dataframes/people-10m.parquet")
...
display(peopleDF.filter($"lastName" like "Z%").select("lastName", "firstName"))
...
peopleDF.select(substring($"lastName", 1, 1).as("lastInitial")).groupBy("lastInitial").count.collect()
...
peopleDF.select(substring($"lastName", 1, 2).as("lastTwoInitials")).groupBy("lastTwoInitials").count().show(10000)
...
peopleDF.select($"firstName", $"lastName").orderBy($"lastName", $"firstName").limit(100).explain()
...
peopleDF.groupBy(substring($"lastName", 1, 1)).count().limit(1000).explain()
...
import org.apache.spark.sql.functions._
val peopleDF = spark.read.option("header", "true").option("inferSchema", "true").option("sep", ":").csv("dbfs:/mnt/training/dataframes/people-with-header-10m.txt") // first DataFrame
val peopleDF2 = peopleDF.select($"*", year($"birthDate").as("birthYear"), year(current_date()).as("thisYear")) // second DataFrame
val peopleDF3 = peopleDF2.filter(($"thisYear" - $"birthYear") <= 30) // third DataFrame
val peopleDF4 = peopleDF3.drop("thisYear") // fourth DataFrame
...
display(peopleDF4.groupBy("birthYear").count())
