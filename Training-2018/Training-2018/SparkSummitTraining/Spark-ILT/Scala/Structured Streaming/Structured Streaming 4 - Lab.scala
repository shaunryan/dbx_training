...
val initialDF = spark.readStream
  ...
  .withColumn("capturedAt", $"epoc".cast("timestamp"))
...
// TODO
// Fill in the missing parts of the code, below.

val ipDF = initialDF
  .withColumn("ip", <<FILL_IN>>) // Extract the IP address
  .filter( <<FILL_IN>> )         // Filter out all records that did not have an IP address
  .groupBy( <<FILL_IN>> )        // Group by capturedAt and ip
  .<<FILL_IN>>                   // Total the number of records for each aggregation
...
display(
  ipDF.select($"window.end".as("time"), $"ip", $"count")
      .orderBy("time", "ip")
)
