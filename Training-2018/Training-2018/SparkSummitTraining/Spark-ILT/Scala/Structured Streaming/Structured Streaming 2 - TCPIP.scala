...
// MAGIC %md
// MAGIC Note that what was just created is a regular DataFrame, with a single `value` column.
// MAGIC 
...
// MAGIC 0. Cast the Java epoch to a SQL-timestamp.
// MAGIC 
...
// MAGIC %md
// MAGIC Again, we can pass this DataFrame to `display()`, to see a continual updating display of the Result Table.
...
// MAGIC 
// MAGIC The problem here is that Spark would have to re-sort, and consequently reshuffle all the data.
// MAGIC 
...
// MAGIC %md
// MAGIC Let's flatten the `window` structure and see what `display()` gives us:
...
val checkpointDir = userhome + "/checkpoint"
...
val streamingQuery = windowedDF
  ...
  .trigger(Trigger.ProcessingTime("20 seconds")) // With a trigger interval of 20 seconds
  .start()                                       // Start the job
...
// MAGIC Using the `streamingQuery` object returned from `DataStreamWriter.start()` we can also check on the status of the query with calls like the ones below:
