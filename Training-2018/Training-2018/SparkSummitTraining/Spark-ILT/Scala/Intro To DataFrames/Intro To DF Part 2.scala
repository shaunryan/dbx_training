...
// MAGIC # print "columnA:", columnA
// MAGIC print("columnC: %s" % columnC)
// MAGIC print("columnD: %s" % columnD)
// MAGIC print("columnE: %s" % columnE)
...
val sortedDescDF = pagecountsEnAllDF
...
val articlesDF = filteredDF
  .drop("bytes_served")
  .filter($"article" =!= "Main_Page")
  .filter($"article" =!= "-")
...
// MAGIC 
// MAGIC At the heart of it, we are simply going to ask for the value of the object in column N via `Column.get(i)`.
// MAGIC 
...
val article = firstRow.getString(1)
val total = firstRow.getInt(2)

printf("Most Requested Article: \"%s\" with %,d requests%n%n", article, total)
...
for (row <- rows) {
  ...
  val total = row.getInt(2)
  listItems += "    <li><b>%s</b> %,d requests</li>%n".format(project, total)
}
...
// MAGIC *is lost with `DataFrame`, or rather, `Dataset[Row]`. Pyton and R have no such concept because they are loosely typed.*
