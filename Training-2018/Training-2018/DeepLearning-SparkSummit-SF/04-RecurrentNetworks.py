...
# MAGIC     * Likely next letter, word, phrase, category, sound, action, value
...
# MAGIC %md ####Effect of "Temperature (Diversity)" on Probabilities
# MAGIC 
# MAGIC ```
# MAGIC preds = np.log(preds) / temperature
# MAGIC exp_preds = np.exp(preds)
# MAGIC preds = exp_preds / np.sum(exp_preds)
# MAGIC ```
# MAGIC 
# MAGIC In this code, the first and second lines take `exp((1/t)ln(p))` for each probability p, where t is the temperature/diversity value.
# MAGIC 
# MAGIC The following graphs help illustrate where that will map different values of p. Because of the (1/t), higher temp/div values correspond to *coefficients* on (ln x) less than 1 in the graph, such as they yellow one for `exp(0.5*ln(x))`. Lower temps correspond to curves below that line.
# MAGIC 
# MAGIC The result is relative difference in amplification of small probabilities. The last line above renormalizes the values so they sum to 1.
# MAGIC 
# MAGIC <img src="https://i.imgur.com/q1dG9Ox.png" width=700>
...
# MAGIC %md ## From Basics to State-of-the-Art
# MAGIC 
# MAGIC To get from our current code to the state of the art in, e.g., language translation, is not as far as you might think. The key elements to add are:
# MAGIC 
# MAGIC * Handling variable length sequences
# MAGIC * Sequence-to-sequence models
# MAGIC   * https://blog.keras.io/a-ten-minute-introduction-to-sequence-to-sequence-learning-in-keras.html
# MAGIC * Using word embeddings instead of character encoding
# MAGIC   * https://www.tensorflow.org/tutorials/word2vec
# MAGIC   * https://www.tensorflow.org/tutorials/seq2seq
# MAGIC * Teacher forcing
# MAGIC   * https://machinelearningmastery.com/teacher-forcing-for-recurrent-neural-networks/
# MAGIC * Adding an attention mechanism to seq2seq
# MAGIC   * http://www.wildml.com/2016/01/attention-and-memory-in-deep-learning-and-nlp/
# MAGIC   * https://github.com/philipperemy/keras-attention-mechanism
