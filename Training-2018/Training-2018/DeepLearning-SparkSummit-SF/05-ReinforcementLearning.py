...
# MAGIC %md __NOTE: these code samples require the `h5py` and `imageio` libraries, so if you haven't already set them up, do so now.__
...
class Catch(object):
    ...
    def _update_state(self, action):
        ...
        if action == 0:  # left
            ...
            action = 1  # right
        brick_row, brick_column, basket_column = self.state
        new_basket = min(max(1, basket_column + action), self.grid_size-1)
        brick_row += 1
        out = np.asarray([brick_row, brick_column, new_basket])
        self.state = out
    ...
    def _draw_state(self):
        ...
        canvas[brick_row, brick_column] = 1  # draw brick
        canvas[-1, basket_column-1:basket_column + 2] = 1  # draw basket
        return canvas

    def _get_reward(self):
        brick_row, brick_column, basket_column = self.state
        if brick_row == self.grid_size-1:
            if abs(brick_column - basket_column) <= 1:
                return 1
            else:
                return -1
        else:
    ...
    def observe(self):
        ...
        return canvas.reshape((1, -1)) #flatten to (1, h*w)
    ...
    def reset(self):
        new_brick_column = np.random.randint(0, self.grid_size-1, size=1)
        new_basket_column = np.random.randint(1, self.grid_size-2, size=1)
        self.state = np.asarray([0, new_brick_column, new_basket_column])
...
class ExperienceReplay(object):
    ...
    def get_batch(self, model, batch_size=10):
        ...
        screen_state_in_first_mem_record = self.memory[0][0][0]
        env_dim = screen_state_in_first_mem_record.shape[1] #get screen size total pixels (flattened)
...
# MAGIC %sh rm -rf /dbfs/keras_rl

# COMMAND ----------

# MAGIC %sh rm /dbfs/FileStore/movie.gif

# COMMAND ----------

# MAGIC %sh mkdir /dbfs/keras_rl

# COMMAND ----------

# MAGIC %sh mkdir /dbfs/keras_rl/images
...
# MAGIC %md ### Concise Overview of Limitations, Challenges, and Guidelines for Success
# MAGIC 
# MAGIC https://www.alexirpan.com/2018/02/14/rl-hard.html
# MAGIC 
