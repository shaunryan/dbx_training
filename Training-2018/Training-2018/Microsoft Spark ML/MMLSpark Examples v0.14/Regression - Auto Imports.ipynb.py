...
# MAGIC %md Declare the schema for the data that will be converted from the pandas
# MAGIC DataFrame to a Spark DataFrame.  Allow all fields to be nullable, so that
# MAGIC missing values can be handled appropriately, such as replacing them with
# MAGIC the mean or median value for that column.
...
# MAGIC %md Read the data from the AutomobilePriceRaw.csv file into a pandas dataframe.
# MAGIC Specify possible reprsentations of missing values, and drop the `num-of-doors`
# MAGIC column as the data is read in.
