...
# MAGIC 
# MAGIC 1. Define a unique directory, `basePath = "{}/etl3p/capstone/".format(userhome)`, to store unparsed and parsed data as well as metadata.
...
basePath = "{}/etl3p/capstone/".format(userhome)
...
dbTest("ET3-P-07-01-01", True, spark.read.format("delta").load(writePath).count() >= 1964)
...
name = "ETL-3-Stream-Job"
notebook_path = "/Shared/ETL-Part-3/Python/Runnable/Runnable-5-Stream" # Your path could be different, likely including your username
...
r = requests.post(RunEndPoint, headers=header, data=json.dumps({"job_id": job_id}))
