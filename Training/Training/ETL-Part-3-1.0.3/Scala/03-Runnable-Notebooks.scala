...
dbutils.widgets.dropdown("MyWidget", "1", Seq("1", "2", "3", "4"))
...
val returnValue = dbutils.notebook.run("./Runnable/Runnable-1", 30)

println(s"Notebook successfully ran with return value: $returnValue")
...
// 
// println(myVariable) // This will fail
...
// MAGIC 
// MAGIC Notebook widgets allow us to pass parameters into notebooks.  This can be done in the form of a map that maps the widget name to a value as a `string`.
// MAGIC 
...
val basePath =  "%s/etl3p/".format(userhome)
val dest_path = "%s/raw_logs.parquet".format(basePath)

val result = dbutils.notebook.run("./Runnable/Runnable-2", 60, Map("date" -> "11-27-2013", "dest_path" -> dest_path))
...
val path = "%s/hour_03.parquet".format(basePath)

dbutils.notebook.run("./Runnable/Runnable-3", 60, Map("hour" -> "03", "output_path" -> path))

// COMMAND ----------

// TEST - Run this cell to test your solution

val r = scala.util.Random.nextInt(scala.math.pow(10, 10).toInt)
val _path = "%s/hour_08_%s.parquet".format(basePath, r)

val _returnValue = dbutils.notebook.run("./Runnable/Runnable-3", 60, Map("hour" -> "08", "output_path" -> _path))
val _df = spark.read.parquet(_returnValue)

dbTest("ET3-S-03-01-01", true, _path == _returnValue)
dbTest("ET3-S-03-01-02", 54206, _df.count)
...
// MAGIC ## Next Steps
// MAGIC 
// MAGIC Start the next lesson, [Scheduling Jobs Programatically]($./04-Scheduling-Jobs-Programatically ).
