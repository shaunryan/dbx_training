...
val pagecountsEnAllDF = spark.read.parquet("/mnt/training/wikipedia/pagecounts/staging_parquet_en_only_clean/") 
...
val uncompressedPath = username + "/academy/pageCountsUncompressed.csv"
val snappyPath = username + "/academy/pageCountsSnappy.csv"
val GZIPPath = username + "/academy/pageCountsGZIP.csv"
...
val uncompressedSize = dbutils.fs.ls(uncompressedPath).map(x => x.size).sum
val snappySize = dbutils.fs.ls(snappyPath).map(x => x.size).sum
val GZIPSize = dbutils.fs.ls(GZIPPath).map(x => x.size).sum
...
def timeIt(df: Dataset[Row], fileType: String, partitions: Int = 1, compression: String="", 
           outputPath: String = username + "/academy/comparisonTest"): Unit = {
  var start = System.currentTimeMillis
  val _df = df.repartition(partitions).write.mode("OVERWRITE")
  
  if (compression.nonEmpty){
    _df.option("compression", compression)
  }
  if (fileType == "csv"){
    _df.csv(outputPath)
  } else {
  ...
  var totalTime = (System.currentTimeMillis - start) / 1000f
  println(s"Save time of ${totalTime}s for fileType: $fileType\tpartitions: $partitions\tcompression: $compression")
  ...
  println(s"\tRead time of ${totalTime}s")
}

def timeAll(df: Dataset[Row], fileTypes: Seq[String], partitionsSeq: Seq[Int], compressions: Seq[String]): Unit = {
  for (fileType <- fileTypes) {
    for (partitions <- partitionsSeq) {
      for (compression <- compressions) {
        timeIt(df, fileType, partitions, compression)
      }
    }
  }
}
...
timeAll(pagecountsEnAllDF, Seq("csv", "parquet"), Seq(1, 16, 32, 64), Seq("", "gzip", "snappy"))
