...
val staging_table = spark.read.parquet("/mnt/training/EDGAR-Log-20170329/enhanced/EDGAR-Log-20170329-sample.parquet/")
  .dropDuplicates("ip", "date", "time")

val production_table = staging_table.sample(.2, seed=123)
...
production_table.count / staging_table.count.toFloat
...
val failedDF = staging_table.join(production_table, Seq("ip", "date", "time"), "left_anti")
...
val fullDF = production_table.union(failedDF)
...
// MAGIC ## Next Steps
// MAGIC 
// MAGIC Start the next lesson, [ETL Optimizations]($./06-ETL-Optimizations ).
