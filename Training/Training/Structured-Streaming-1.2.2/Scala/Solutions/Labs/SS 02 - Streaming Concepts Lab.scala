...
var dataPath = "dbfs:/mnt/training/asa/flights/2007-01-stream.parquet"
...
val delaysDF = initialDF
  .withColumn("CarrierDelay", $"CarrierDelay".cast("integer"))
  .withColumn("WeatherDelay", $"WeatherDelay".cast("integer"))
  .withColumn("NASDelay", $"NASDelay".cast("integer"))
  .withColumn("SecurityDelay", $"SecurityDelay".cast("integer"))
  .withColumn("LateAircraftDelay", $"LateAircraftDelay".cast("integer"))
  .withColumn("TotalDelay", $"CarrierDelay" + $"WeatherDelay" + $"NASDelay" + $"SecurityDelay" + $"LateAircraftDelay")
  .filter($"UniqueCarrier".isin("AS", "AQ", "HA", "F9"))
  .filter($"TotalDelay" > 0)
...
// ANSWER
display(delaysDF, streamName = "delays_scala")
...
val delayQuery = delaysDF  
 ...
 .queryName("delays_scala")                     // Name the query "delays_scala"
 .trigger(Trigger.ProcessingTime("15 seconds")) // Use a 15 second trigger
 .start()                                       // Trigger execution of the query with "activityPath"
...
for (s <- spark.streams.active) { // Iterate over all active streams
  println("stopping " + s.name)   // A little console output
