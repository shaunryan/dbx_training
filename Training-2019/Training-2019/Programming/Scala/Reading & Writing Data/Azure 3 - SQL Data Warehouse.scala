...
spark.conf.set("fs.azure.account.key.%s.blob.core.windows.net".format(storageAccount), accessKey)
...
val cacheDir = "wasbs://%s@%s.blob.core.windows.net/cacheDir".format(containerName, storageAccount)
...
spark.sql("select count(*) from customer_data")

// COMMAND ----------

spark.sql("describe customer_data")
...
spark.sql("select CustomerKey, CustomerAlternateKey from customer_data limit 10")
...
spark.sql("select CustomerKey, CustomerAlternateKey from customer_temp_data limit 10")
