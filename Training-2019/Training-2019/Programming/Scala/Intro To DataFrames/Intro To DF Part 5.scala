...
val columnTrans = date_format($"capturedAt", "u").alias("dow")

val pageviewsWithDowDF = pageviewsDF
...
val labelsDF = spark.read.parquet(source + "/day-of-week")
...
val joinedDowDF = pageviewsWithDowDF
...
val aggregatedDowDF = joinedDowDF
...
printf("Threshold: %,d%n%n", threshold )
