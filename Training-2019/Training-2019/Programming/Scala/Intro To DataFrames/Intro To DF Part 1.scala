...
val path = source + "/wikipedia/pagecounts/staging_parquet_en_only_clean/"
...
printf("Record Count: %,d%n%n".format(total))
...
val limitedDF = pagecountsEnAllDF.limit(5) // "limit" the number of records to the first 5
...
val onlyThreeDF = pagecountsEnAllDF
...
// Transform the data by dropping
val droppedDF = pagecountsEnAllDF
...
val distinctDF = pagecountsEnAllDF  // Our original DataFrame from spark.read.parquet(..)
...
val total = distinctDF.count()     
printf("Distinct Projects: %,d%n%n", total)
...
val tableDF = spark.sql("SELECT DISTINCT project FROM pagecounts ORDER BY project")
display(tableDF)
