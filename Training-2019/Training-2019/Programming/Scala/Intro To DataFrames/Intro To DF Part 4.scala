...
val parquetFile = source + "/wikipedia/pageviews/pageviews_by_second.parquet/"
...
val byDayOfWeekDF = pageviewsDF
  .groupBy( date_format($"capturedAt", "E") )      // format as Mon, Tue and then aggregate
  .sum()                                           // produce the sum of all records
  .select($"date_format(capturedAt, E)".as("dow"), // rename to "dow"
          $"sum(requests)".as("total"))            // add some commas and rename to "total"
...
def mapDayOfWeek(day:String): String = {
  day match {
    case "Mon" => "1-Mon"
    case "Tue" => "2-Tue"
    case "Wed" => "3-Wed"
...
val prependNumberUDF = spark.udf.register("prependNumber", mapDayOfWeek _)
...
display(
  byDayOfWeekDF
    .select( prependNumberUDF($"dow").as("Day Of Week"),  $"total" )
    .orderBy("Day Of Week")
)
...
display(
  pageviewsDF
    .groupBy( date_format($"capturedAt", "u-E").alias("Day Of Week") )
    .sum()
    .orderBy($"Day Of Week")
)
...
val mobileDF = pageviewsDF
    .filter($"site" === "mobile")
    .groupBy( date_format($"capturedAt", "u-E").as("Day Of Week") )
    .sum()
    .withColumnRenamed("sum(requests)", "Mobile Total")
    .orderBy($"Day Of Week")

val desktopDF = pageviewsDF
    .filter($"site" === "desktop")
    .groupBy( date_format($"capturedAt", "u-E").as("Day Of Week") )
    .sum()
...
val columnA = desktopDF("Day Of Week")
val columnB = mobileDF("Day Of Week")
...
val tempDF = desktopDF.join(mobileDF, desktopDF("Day Of Week") === mobileDF("Day Of Week"))
...
val joinedDF = tempDF
  ...
  .select($"Day Of Week", $"Desktop Total", $"Mobile Total")
...
val altDF = desktopDF.join(mobileDF, "Day Of Week")
