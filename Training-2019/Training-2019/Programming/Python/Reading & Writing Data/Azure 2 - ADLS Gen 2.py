...
dbutils.fs.mount(
  ...
  mount_point = "/mnt/{}-{}".format(username, fileSystemName),
...
files = dbutils.fs.ls("/mnt/{}-{}".format(username, fileSystemName))
display(files)

# COMMAND ----------

wikiEditsDF = spark.read.json("/mnt/{}-{}/wikipedia/edits/snapshot-2016-05-26.json".format(username, fileSystemName))
display(wikiEditsDF)
...
(wikiEditsDF.write
  ...
  .format("delta")
  .partitionBy("channel")
  .save("/mnt/{}-{}/wikiEdits".format(username, fileSystemName)))
...
dbutils.fs.unmount("/mnt/{}-{}".format(username, fileSystemName))
