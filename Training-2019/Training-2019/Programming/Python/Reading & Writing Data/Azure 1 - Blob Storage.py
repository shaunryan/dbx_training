...
# TODO
storageAccountName = FILL_IN
containerName = FILL_IN
storageEntity = "fs.azure.account.key.{}.blob.core.windows.net".format(storageAccountName)
accessKey = FILL_IN
...
sourceURI = "wasbs://{}@{}.blob.core.windows.net/".format(containerName, storageAccountName)
mountPath = "/mnt/{}-{}".format(username, containerName)
...
files = dbutils.fs.ls("/mnt/{}-{}".format(username, containerName))
display(files)
...
path = "{}/azblob/wikiEdits.parquet".format(userhome)
...
dbutils.fs.unmount("/mnt/{}-{}".format(username, containerName))
