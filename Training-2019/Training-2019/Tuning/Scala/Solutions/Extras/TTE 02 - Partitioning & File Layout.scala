...
// MAGIC  - Partition your data for increased query performance
// MAGIC  - Minimize the small file problem
...
val df = (spark
          ...
          .csv("/mnt/training/dataframes/people-10m.csv"))
...
val df_by_year = (spark
                  .read
                  .option("header", "true")
                  .option("inferSchema", "true")
                  .csv("/mnt/training/dataframes/people-10m-partitioned.csv"))
...
val tableName = username.replaceAll("\\W", "") + "people_by_year_optimized"
...
display(dbutils.fs.ls(userhome + "/tuning-s/people_by_year.parquet/birthYear=1979"))
