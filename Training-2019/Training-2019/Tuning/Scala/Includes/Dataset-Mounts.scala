...
// MAGIC val sparkVersions = tags.collect({ case (t, v) if t.name == "sparkVersion" => v })
// MAGIC val version = if (sparkVersions.isEmpty == false) {
// MAGIC   sparkVersions.head  
// MAGIC } else {
// MAGIC   throw new RuntimeException("The workspace was not quite ready... Please try running this command again.")
// MAGIC }
// MAGIC 
...
// MAGIC   try {
// MAGIC     dbutils.notebook.getContext().tags("notebookId").toLong
// MAGIC   } catch {
// MAGIC     case _: Throwable => dbutils.notebook.getContext().tags("jobId").toLong
// MAGIC   }
// MAGIC }
// MAGIC 
