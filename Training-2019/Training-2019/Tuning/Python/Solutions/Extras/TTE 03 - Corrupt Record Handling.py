...
# MAGIC  - Examine the 3 ways Spark allows you to handle corrupt data
# MAGIC  - Use the Databricks `badRecordsPath` to write out all your corrupt data
...
corruptDF = (spark.read
  .option("badRecordsPath", userhome + "/tuning-p/badRecordsPath")
  .json(sc.parallelize(data))
)
display(corruptDF)
...
display(spark.read.json(userhome + "/tuning-p/badRecordsPath/*/*/*"))
...
SMSCorruptDF2 = (spark.read
  .option("badRecordsPath", userhome + "/tuning-p/corruptSMS")
  .json("/mnt/training/UbiqLog4UCI/14_F/log*")
)
