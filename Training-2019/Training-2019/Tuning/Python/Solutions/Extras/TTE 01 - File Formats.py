...
# MAGIC - Compare CSV to Parquet in terms of storage and query performance
# MAGIC - Analyze the impact of different file compression formats on parallelization of tasks
...
csvDF = (spark
         .read
         .csv("mnt/training/dataframes/people-with-header-10m.txt", header="true", sep=":"))
...
csvDF = (spark
         .read
         .csv("mnt/training/dataframes/people-with-header-10m.txt", header="true", sep=":", inferSchema="true"))
...
csvDFsnappy = (spark
               .read
               .csv("mnt/training/dataframes/people-with-header-10m.txt.snappy", header="true", sep=":", inferSchema="true"))
...
csvBzip = (spark
           .read
           .csv("mnt/training/dataframes/tuning/people-with-header-10m.csv.bzip", header=True, sep=":", inferSchema=True))
...
csvDFgz = (spark
         .read
         .csv("mnt/training/dataframes/people-with-header-10m.txt.gz", header="true", sep=":", inferSchema="true"))
...
schema_json = dbutils.fs.head(userhome + "/tmp/myschema.json", 1024)
...
parquetDF = spark.read.parquet("mnt/training/dataframes/people-10m.parquet/")
