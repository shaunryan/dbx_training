...
dbutils.fs.rm(userhome + "/tuning-p/iot-pipeline/", True)
...
rawData.write.mode("overwrite").format("parquet").partitionBy("date").save(userhome + "/tuning-p/iot-pipeline/")
...
tableName = userSub + "_demo_iot_data_p"
tablePath = userhome + "/tuning-p/iot-pipeline/"

sql("DROP TABLE IF EXISTS {}".format(tableName))

sql("""
CREATE TABLE {} (action STRING, deviceId INTEGER, date DATE)
USING parquet
OPTIONS (path = '{}')
PARTITIONED BY (date)
...
display(dbutils.fs.ls(userhome + "/tuning-p/iot-pipeline/"))
...
display(sql("MSCK REPAIR TABLE {}".format(tableName)))
...
new_data.write.format("parquet").partitionBy("date").mode("append").save(userhome + "/tuning-p/iot-pipeline/")
...
(new_data
   ...
   .format("parquet")
   .mode("overwrite")
   .save(userhome + "/tuning-p/iot-pipeline/date=2018-01-30/")
)
...
try:
  ...
  sql("SELECT count(*) FROM {}".format(tableName)).count()
...
display(sql("REFRESH TABLE {}".format(tableName)))
...
(old_batch_data
 ...
 .partitionBy("date")
 .mode("append")
 .save(userhome + "/tuning-p/iot-pipeline/")
)
...
sql("MSCK REPAIR TABLE {}".format(tableName))
