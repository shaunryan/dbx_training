...
spark.conf.set(FILL_IN, FILL_IN)

val streamDF = spark  // Start with the SparkSesion   
  .FILL_IN            // Get the DataStreamReader
  .FILL_IN            // Configure the stream's source for the appropriate file type
  .FILL_IN            // Specify the parquet files' schema
  .FILL_IN            // Restrict Spark to processing only 1 file per trigger
  .FILL_IN            // Load the DataFrame specifying its location with dataPath
...
// TODO
import org.apache.spark.sql.functions.{FILL_IN}

val countsDF = streamDF  // Start with the DataFrame
  .FILL_IN               // Specify the watermark
  .FILL_IN               // Aggregate the data
  .FILL_IN               // Produce a count for each aggreate
  .FILL_IN               // Add the column "hour", extracting it from "window.start"

val myStream = "SS03L_1S"
display(countsDF,  streamName = myStream)
