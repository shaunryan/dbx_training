...
// ANSWER
spark.sql(s"""
    CREATE TABLE IF NOT EXISTS %s.mobile_events_delta_bronze
    USING DELTA 
    LOCATION "%s" 
...
lazy val tableExists = spark.catalog.tableExists("%s.mobile_events_delta_bronze".format(databaseName))
lazy val firstRow = spark.sql("SELECT * FROM %s.mobile_events_delta_bronze".format(databaseName)).take(1)
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS %s.device_id_type_table 
    USING DELTA
    LOCATION "%s" """.format(databaseName, tablePath))

// COMMAND ----------

// TEST - Run this cell to test your solution.
val tableExists = spark.catalog.tableExists("%s.device_id_type_table".format(databaseName))
...
lazy val schema = spark.table("%s.mobile_events_delta_silver".format(databaseName)).schema.mkString(",")
...
lazy val firstRowQuery = spark.sql("SELECT * FROM %s.mobile_events_delta_silver".format(databaseName)).take(1)
...
// ANSWER
spark.sql(s"""
    CREATE TABLE IF NOT EXISTS %s.mobile_events_delta_gold 
    USING DELTA
    LOCATION "%s"
    AS
      SELECT count(DISTINCT deviceId) AS WAU, weekofyear(eventTime) as week
      FROM %s.mobile_events_delta_silver
...
// TEST - Run this cell to test your solution.
lazy val schema = spark.table("%s.mobile_events_delta_gold".format(databaseName)).schema.mkString(",")
...
val platinumDF = spark.sql("""
   SELECT * FROM %s.mobile_events_delta_gold
   """.format(databaseName))
...
// ANSWER  
val deviceId = spark.sql("SELECT deviceId FROM %s.mobile_events_delta_silver limit 1".format(databaseName)).first()(0).toString
...
spark.sql(s"""OPTIMIZE %s.mobile_events_delta_silver
...
// ANSWER
spark.sql("DELETE FROM %s.mobile_events_delta_silver WHERE deviceId='%s' ".format(databaseName, deviceId))
val noDeviceId = spark.sql("SELECT * FROM %s.mobile_events_delta_silver WHERE deviceId='%s' ".format(databaseName, deviceId)).collect()
...
// ANSWER
spark.sql("VACUUM %s.mobile_events_delta_silver RETAIN 0 HOURS".format(databaseName))
...
var numFilesOne = 0
try {
  numFilesOne  = dbutils.fs.ls(outputPathSilver) 
