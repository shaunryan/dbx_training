...
val bronzeDF = spark.sql("SELECT * FROM delta.`%s` limit 1".format(bronzePath))
display(bronzeDF)
...
val silverDF = spark.sql("SELECT * FROM delta.`%s` limit 3".format(silverPath))
display(silverDF)
...
// MAGIC %run "./Includes/Classroom-Cleanup-07"
