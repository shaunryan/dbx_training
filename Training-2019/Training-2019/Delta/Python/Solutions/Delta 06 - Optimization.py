...
deltaDataPath = basePath + "/customer-data-delta/"
...
# MAGIC 
# MAGIC  In Python we use the `timeit` 
# MAGIC <a href="https://docs.python.org/2/library/timeit.html" target="_blank">library function</a> 
# MAGIC 
# MAGIC 
# MAGIC 
...
def myQuery():
  return """spark.sql("SELECT * FROM delta.`{}` WHERE StockCode=22301 ".format(deltaDataPath)).collect()"""
...
postTime = timeit.timeit(myQuery)
...
# ANSWER
spark.sql("VACUUM delta.`{}` RETAIN 0 HOURS".format(deltaDataPath))
