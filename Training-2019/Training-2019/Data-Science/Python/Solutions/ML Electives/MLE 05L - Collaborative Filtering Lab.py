...
moviesDF = spark.read.parquet("dbfs:/mnt/training/movielens/movies.parquet/").cache()
ratingsDF = spark.read.parquet("dbfs:/mnt/training/movielens/ratings.parquet/").cache()
...
seed = 42
cv = CrossValidator(numFolds=3, estimator=als, estimatorParamMaps=grid, evaluator=regEval, seed=seed)          
...
print(f"The model had a RMSE on the test set of {testRmse:.3}")
