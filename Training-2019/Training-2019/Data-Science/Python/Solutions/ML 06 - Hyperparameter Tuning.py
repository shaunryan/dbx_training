...
# MAGIC 
# MAGIC Before we get to random forests, let's look at some of the options we have available to improve our decision tree model. If you already have `mlflow` 1.0.0+ installed and are using Databricks Runtime for ML 5.4+, using the SparkML cross-validator will automatically log your experiments to MLflow!
# MAGIC 
...
for categoricalCol in categoricalColumns:
    stages.append(StringIndexer(inputCol=categoricalCol, outputCol=categoricalCol + "Index", handleInvalid="skip"))
...
# MAGIC %md
# MAGIC 
# MAGIC Fascinating! It appears to do better with a shallower tree instead of a deeper tree. Let's see how it does on the test dataset.
