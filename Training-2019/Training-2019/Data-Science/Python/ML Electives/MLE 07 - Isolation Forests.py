...
# MAGIC * Use Logisitic Regression and a Random Forest to determine feature importance using spark-sklearn
# MAGIC 
...
# MAGIC %md
# MAGIC We see that having a nonzero transfer amount with a zero change in destination account balance seems to indicate fraud. We'll create a feature that captures the difference between the amount transfered and the actual change in destination account balance.
...
# MAGIC 
# MAGIC We will first calculate contaimination for this dataset.

# COMMAND ----------

# MAGIC %python
# MAGIC fraudContaimination = fraudDF.filter("isFraud = 1").count() / fraudDF.count()
# MAGIC print(f"Contaimination for fraud dataset {fraudContaimination:.4f}.")
...
# MAGIC %md
# MAGIC ## Feature Importance using Logisitic Regression
# MAGIC [Logistic Regression](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html#sklearn.linear_model.LogisticRegression) is a linear model for classification that models the probabilities of possible outcomes using a logisitic function.
# MAGIC 
# MAGIC Isolation forests do not have a built-in feature importance function; however, we can infer feature importances by training a model that uses the predictions of the isolation forest as labels and the features used as inputs. We can use logisitic regression to predict if the isolation forest would mark the transaction as fraud or not, and we can treat the feature importances of the logisitic regression model as the feature importance of the isolation forest. By using a linear model, we get both importance and direction (i.e. increasing the feature value leads to the model predicting the transaction as more or less likely to be fraudulent)
# MAGIC 
