...
val labelDF = airbnbDF.select(when($"host_is_superhost" === "t", 1.0).otherwise(0.0).alias("label"), $"*").drop("host_is_superhost")

val predDF = labelDF.withColumn("prediction", lit(0.0))
...
val rFormula = new RFormula()
  ...
  .setLabelCol("label")
  .setHandleInvalid("skip") // Look at handleInvalid
...
println(f"The area under the PR curve: ${bcEvaluator.evaluate(predDF)}%1.2f")
...
val paramGrid = (new ParamGridBuilder()
                    .addGrid(lr.regParam, Array(0.1, 0.2, 1.0))
                    .addGrid(lr.elasticNetParam, Array(0.0, 0.5, 1.0))
                    .build())

val cv = (new CrossValidator()
              .setEstimator(lr)
              .setEvaluator(mcEvaluator)
              .setEstimatorParamMaps(paramGrid)
              .setNumFolds(3)
              .setParallelism(4)
              .setSeed(42))

val pipeline = new Pipeline().setStages(Array(rFormula, cv))
