...
# MAGIC * [Horovod Runner Docs](https://docs.azuredatabricks.net/applications/deep-learning/distributed-training/horovod-runner.html#horovodrunner)
...
## OPTIONAL: You can enable Horovod Timeline as follows, but can incur up to 40% slow down from frequent writes
# import os
# os.environ['HOROVOD_TIMELINE'] = f"/dbfs/ml/{ml_username}_timeline.json"
...
# MAGIC %md
# MAGIC If you want to access the [Hovorod Timeline](https://docs.azuredatabricks.net/applications/deep-learning/distributed-training/horovod-runner.html#distributed-training-with-horovodrunner), export file out of Databricks by copying to the FileStore, then loading it in `chrome://tracing/`.

# COMMAND ----------

# dbutils.fs.cp(f"/dbfs/ml/{ml_username}_timeline.json", f"dbfs:/FileStore/{ml_username}_timeline.json")
...
def run_training_horovod():
  ...
  checkpoint_dir = f"/dbfs/ml/{ml_username}/horovod_checkpoint_weights.ckpt"
